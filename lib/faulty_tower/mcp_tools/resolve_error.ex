defmodule FaultyTower.MCPTools.ResolveError do
  use Vancouver.Tool

  alias FaultyTower.Errors

  def name, do: "resolve_error"
  def description, do: "Mark an error as resolved"
  ...
  def input_schema do
    %{
      ...
      "properties" => %{
        "error_id" => %{
          "type" => "string",
          "description" => "The error ID to resolve"
        }
      },
      "required" => ["error_id"]
    }
  ...
  def run(conn, params) do
    ...
    case Errors.get_user_error(user, String.to_integer(error_id)) do
      {:ok, error} ->
        case Errors.resolve_error(error) do
          {:ok, updated_error} ->
          ...
          {:error, changeset} ->
            send_error(conn, "Failed to resolve error: #{inspect(changeset.errors)}")
        end
